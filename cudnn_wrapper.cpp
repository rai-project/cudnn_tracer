...
cudnnStatus_t CUDNNWINAPI cudnnQueryRuntimeError(
   ...
   static fun_t * orig_cudnnQueryRuntimeError = (fun_t *) (dlsym(RTLD_NEXT,"cudnnQueryRuntimeError"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetStream     (cudnnHandle_t handle, cudaStream_t streamId) {
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"streamId", convert_to_json(streamId)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnCreateTensorDescriptor(
   ...
   static fun_t * orig_cudnnCreateTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateTensorDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetTensor4dDescriptor(
   ...
   static fun_t * orig_cudnnSetTensor4dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetTensor4dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetTensor4dDescriptor(tensorDesc, format, dataType, n, c, h, w);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"format", convert_to_json(format)},
              {"dataType", convert_to_json(dataType)},
              {"c", convert_to_json(c)},
              {"w", convert_to_json(w)}
       }}
      };
...
cudnnStatus_t CUDNNWINAPI cudnnSetTensor4dDescriptorEx(
   ...
   static fun_t * orig_cudnnSetTensor4dDescriptorEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetTensor4dDescriptorEx"));

   const auto tic = now();
   const auto res = orig_cudnnSetTensor4dDescriptorEx(tensorDesc, dataType, n, c, h, w, nStride, cStride, hStride, wStride);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"dataType", convert_to_json(dataType)},
              {"c", convert_to_json(c)},
              {"nStride", convert_to_json(nStride)},
              {"cStride", convert_to_json(cStride)},
              {"hStride", convert_to_json(hStride)},
              {"wStride", convert_to_json(wStride)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetTensor4dDescriptor(
   ...
   static fun_t * orig_cudnnGetTensor4dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetTensor4dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetTensor4dDescriptor(tensorDesc, dataType, n, c, h, w, nStride, cStride, hStride, wStride);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetTensorNdDescriptor(
   ...
   static fun_t * orig_cudnnSetTensorNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetTensorNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetTensorNdDescriptor(tensorDesc, dataType, nbDims, dimA, strideA);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"arguments", {
       {"dataType", convert_to_json(dataType)},
              {"nbDims", convert_to_json(nbDims)},
              {"dimA", convert_to_json(dimA)},
              {"strideA", convert_to_json(strideA)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnSetTensorNdDescriptorEx(
   ...
   static fun_t * orig_cudnnSetTensorNdDescriptorEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetTensorNdDescriptorEx"));

   const auto tic = now();
   const auto res = orig_cudnnSetTensorNdDescriptorEx(tensorDesc, format, dataType, nbDims, dimA);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetTensorNdDescriptor(
   ...
   static fun_t * orig_cudnnGetTensorNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetTensorNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetTensorNdDescriptor(tensorDesc, nbDimsRequested, dataType, nbDims, dimA, strideA);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"nbDimsRequested", convert_to_json(nbDimsRequested)},
              {"dimA", convert_to_json(dimA)},
              {"strideA", convert_to_json(strideA)}
       }}
      };
...
cudnnStatus_t CUDNNWINAPI cudnnGetTensorSizeInBytes(
   ...
   static fun_t * orig_cudnnGetTensorSizeInBytes = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetTensorSizeInBytes"));
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyTensorDescriptor(
   ...
   static fun_t * orig_cudnnDestroyTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyTensorDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnTransformTensor(
   ...
   static fun_t * orig_cudnnTransformTensor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnTransformTensor"));
...
cudnnStatus_t CUDNNWINAPI cudnnCreateOpTensorDescriptor(
   ...
   static fun_t * orig_cudnnCreateOpTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateOpTensorDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetOpTensorDescriptor(
   ...
   static fun_t * orig_cudnnSetOpTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetOpTensorDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetOpTensorDescriptor(opTensorDesc, opTensorOp, opTensorCompType, opTensorNanOpt);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"opTensorOp", convert_to_json(opTensorOp)},
              {"opTensorCompType", convert_to_json(opTensorCompType)},
              {"opTensorNanOpt", convert_to_json(opTensorNanOpt)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetOpTensorDescriptor(
   ...
   static fun_t * orig_cudnnGetOpTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetOpTensorDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetOpTensorDescriptor(opTensorDesc, opTensorOp, opTensorCompType, opTensorNanOpt);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyOpTensorDescriptor(
   ...
   static fun_t * orig_cudnnDestroyOpTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyOpTensorDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnOpTensor(
   ...
   const auto tic = now();
   const auto res = orig_cudnnOpTensor(handle, opTensorDesc, alpha1, aDesc, A, alpha2, bDesc, B, beta, cDesc, C);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateReduceTensorDescriptor(
   ...
   static fun_t * orig_cudnnCreateReduceTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateReduceTensorDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetReduceTensorDescriptor(
   ...
   static fun_t * orig_cudnnSetReduceTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetReduceTensorDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetReduceTensorDescriptor(reduceTensorDesc, reduceTensorOp, reduceTensorCompType, reduceTensorNanOpt, reduceTensorIndices, reduceTensorIndicesType);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"reduceTensorOp", convert_to_json(reduceTensorOp)},
              {"reduceTensorCompType", convert_to_json(reduceTensorCompType)},
              {"reduceTensorNanOpt", convert_to_json(reduceTensorNanOpt)},
              {"reduceTensorIndices", convert_to_json(reduceTensorIndices)},
              {"reduceTensorIndicesType", convert_to_json(reduceTensorIndicesType)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetReduceTensorDescriptor(
   ...
   static fun_t * orig_cudnnGetReduceTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetReduceTensorDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetReduceTensorDescriptor(reduceTensorDesc, reduceTensorOp, reduceTensorCompType, reduceTensorNanOpt, reduceTensorIndices, reduceTensorIndicesType);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyReduceTensorDescriptor(
   ...
   static fun_t * orig_cudnnDestroyReduceTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyReduceTensorDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnGetReductionIndicesSize(
   ...
   static fun_t * orig_cudnnGetReductionIndicesSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetReductionIndicesSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetReductionIndicesSize(handle, reduceTensorDesc, aDesc, cDesc, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetReductionWorkspaceSize(
   ...
   static fun_t * orig_cudnnGetReductionWorkspaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetReductionWorkspaceSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetReductionWorkspaceSize(handle, reduceTensorDesc, aDesc, cDesc, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnReduceTensor(
   ...
   const auto tic = now();
   const auto res = orig_cudnnReduceTensor(handle, reduceTensorDesc, indices, indicesSizeInBytes, workspace, workspaceSizeInBytes, alpha, aDesc, A, beta, cDesc, C);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"indicesSizeInBytes", convert_to_json(indicesSizeInBytes)},
              {"workspaceSizeInBytes", convert_to_json(workspaceSizeInBytes)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnCreateFilterDescriptor(
   ...
   static fun_t * orig_cudnnCreateFilterDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateFilterDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetFilter4dDescriptor(
   ...
   static fun_t * orig_cudnnSetFilter4dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetFilter4dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetFilter4dDescriptor(filterDesc, dataType, format, k, c, h, w);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetFilter4dDescriptor(
   ...
   static fun_t * orig_cudnnGetFilter4dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetFilter4dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetFilter4dDescriptor(filterDesc, dataType, format, k, c, h, w);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetFilterNdDescriptor(
   ...
   static fun_t * orig_cudnnSetFilterNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetFilterNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetFilterNdDescriptor(filterDesc, dataType, format, nbDims, filterDimA);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"dataType", convert_to_json(dataType)},
              {"format", convert_to_json(format)},
              {"nbDims", convert_to_json(nbDims)},
              {"filterDimA", convert_to_json(filterDimA)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetFilterNdDescriptor(
   ...
   static fun_t * orig_cudnnGetFilterNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetFilterNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetFilterNdDescriptor(filterDesc, nbDimsRequested, dataType, format, nbDims, filterDimA);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyFilterDescriptor(
   ...
   static fun_t * orig_cudnnDestroyFilterDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyFilterDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnCreateConvolutionDescriptor(
   ...
   static fun_t * orig_cudnnCreateConvolutionDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateConvolutionDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetConvolutionMathType( cudnnConvolutionDescriptor_t convDesc,
   ...
   static fun_t * orig_cudnnSetConvolutionMathType = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetConvolutionMathType"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"mathType", convert_to_json(mathType)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionMathType( cudnnConvolutionDescriptor_t convDesc,
   ...
   static fun_t * orig_cudnnGetConvolutionMathType = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionMathType"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetConvolutionGroupCount( cudnnConvolutionDescriptor_t convDesc,
   ...
   static fun_t * orig_cudnnSetConvolutionGroupCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetConvolutionGroupCount"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"groupCount", convert_to_json(groupCount)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionGroupCount( cudnnConvolutionDescriptor_t convDesc,
   ...
   static fun_t * orig_cudnnGetConvolutionGroupCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionGroupCount"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetConvolution2dDescriptor( cudnnConvolutionDescriptor_t convDesc,
   ...
   static fun_t * orig_cudnnSetConvolution2dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetConvolution2dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetConvolution2dDescriptor(convDesc, pad_h, pad_w, u, v, dilation_h, dilation_w, mode, computeType);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"pad_h", convert_to_json(pad_h)},
              {"pad_w", convert_to_json(pad_w)},
              {"u", convert_to_json(u)},
              {"dilation_h", convert_to_json(dilation_h)},
              {"dilation_w", convert_to_json(dilation_w)},
              {"mode", convert_to_json(mode)},
              {"computeType", convert_to_json(computeType)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolution2dDescriptor(  const cudnnConvolutionDescriptor_t convDesc,
   ...
   static fun_t * orig_cudnnGetConvolution2dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolution2dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolution2dDescriptor(convDesc, pad_h, pad_w, u, v, dilation_h, dilation_w, mode, computeType);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolution2dForwardOutputDim(
   ...
   static fun_t * orig_cudnnGetConvolution2dForwardOutputDim = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolution2dForwardOutputDim"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolution2dForwardOutputDim(convDesc, inputTensorDesc, filterDesc, n, c, h, w);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetConvolutionNdDescriptor(
   ...
   static fun_t * orig_cudnnSetConvolutionNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetConvolutionNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetConvolutionNdDescriptor(convDesc, arrayLength, padA, filterStrideA, dilationA, mode, computeType);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"arrayLength", convert_to_json(arrayLength)},
              {"padA", convert_to_json(padA)},
              {"filterStrideA", convert_to_json(filterStrideA)},
              {"dilationA", convert_to_json(dilationA)},
              {"mode", convert_to_json(mode)},
              {"computeType", convert_to_json(computeType)}
       }}
      };
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionNdDescriptor(
   ...
   static fun_t * orig_cudnnGetConvolutionNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionNdDescriptor(convDesc, arrayLengthRequested, arrayLength, padA, strideA, dilationA, mode, computeType);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"arrayLengthRequested", convert_to_json(arrayLengthRequested)},
              {"padA", convert_to_json(padA)},
              {"strideA", convert_to_json(strideA)},
              {"dilationA", convert_to_json(dilationA)}
       }}
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionNdForwardOutputDim(
   ...
   static fun_t * orig_cudnnGetConvolutionNdForwardOutputDim = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionNdForwardOutputDim"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionNdForwardOutputDim(convDesc, inputTensorDesc, filterDesc, nbDims, tensorOuputDimA);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"arguments", {
       {"nbDims", convert_to_json(nbDims)},
              {"tensorOuputDimA", convert_to_json(tensorOuputDimA)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyConvolutionDescriptor(
   ...
   static fun_t * orig_cudnnDestroyConvolutionDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyConvolutionDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionForwardAlgorithmMaxCount( cudnnHandle_t     handle,
   ...
   static fun_t * orig_cudnnGetConvolutionForwardAlgorithmMaxCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionForwardAlgorithmMaxCount"));
...
cudnnStatus_t CUDNNWINAPI cudnnFindConvolutionForwardAlgorithm(
   ...
   static fun_t * orig_cudnnFindConvolutionForwardAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindConvolutionForwardAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnFindConvolutionForwardAlgorithm(handle, xDesc, wDesc, convDesc, yDesc, requestedAlgoCount, returnedAlgoCount, perfResults);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"requestedAlgoCount", convert_to_json(requestedAlgoCount)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnFindConvolutionForwardAlgorithmEx(
   ...
   static fun_t * orig_cudnnFindConvolutionForwardAlgorithmEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindConvolutionForwardAlgorithmEx"));

   const auto tic = now();
   const auto res = orig_cudnnFindConvolutionForwardAlgorithmEx(handle, xDesc, x, wDesc, w, convDesc, yDesc, y, requestedAlgoCount, returnedAlgoCount, perfResults, workSpace, workSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionForwardAlgorithm(
   ...
   static fun_t * orig_cudnnGetConvolutionForwardAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionForwardAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionForwardAlgorithm(handle, xDesc, wDesc, convDesc, yDesc, preference, memoryLimitInBytes, algo);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"preference", convert_to_json(preference)},
              {"memoryLimitInBytes", convert_to_json(memoryLimitInBytes)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionForwardAlgorithm_v7(
   ...
   static fun_t * orig_cudnnGetConvolutionForwardAlgorithm_v7 = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionForwardAlgorithm_v7"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionForwardAlgorithm_v7(handle, srcDesc, filterDesc, convDesc, destDesc, requestedAlgoCount, returnedAlgoCount, perfResults);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionForwardWorkspaceSize(
   ...
   static fun_t * orig_cudnnGetConvolutionForwardWorkspaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionForwardWorkspaceSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionForwardWorkspaceSize(handle, xDesc, wDesc, convDesc, yDesc, algo, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnConvolutionForward(
   ...
   static fun_t * orig_cudnnConvolutionForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnConvolutionForward"));

   const auto tic = now();
   const auto res = orig_cudnnConvolutionForward(handle, alpha, xDesc, x, wDesc, w, convDesc, algo, workSpace, workSpaceSizeInBytes, beta, yDesc, y);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnConvolutionBiasActivationForward(
   ...
   static fun_t * orig_cudnnConvolutionBiasActivationForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnConvolutionBiasActivationForward"));

   const auto tic = now();
   const auto res = orig_cudnnConvolutionBiasActivationForward(handle, alpha1, xDesc, x, wDesc, w, convDesc, algo, workSpace, workSpaceSizeInBytes, alpha2, zDesc, z, biasDesc, bias, activationDesc, yDesc, y);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnConvolutionBackwardBias(
   ...
   static fun_t * orig_cudnnConvolutionBackwardBias = (fun_t *) (dlsym(RTLD_NEXT,"cudnnConvolutionBackwardBias"));

   const auto tic = now();
   const auto res = orig_cudnnConvolutionBackwardBias(handle, alpha, dyDesc, dy, beta, dbDesc, db);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardFilterAlgorithmMaxCount( cudnnHandle_t     handle,
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardFilterAlgorithmMaxCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardFilterAlgorithmMaxCount"));
...
cudnnStatus_t CUDNNWINAPI cudnnFindConvolutionBackwardFilterAlgorithm(
   ...
   static fun_t * orig_cudnnFindConvolutionBackwardFilterAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindConvolutionBackwardFilterAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnFindConvolutionBackwardFilterAlgorithm(handle, xDesc, dyDesc, convDesc, dwDesc, requestedAlgoCount, returnedAlgoCount, perfResults);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnFindConvolutionBackwardFilterAlgorithmEx(
   ...
   static fun_t * orig_cudnnFindConvolutionBackwardFilterAlgorithmEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindConvolutionBackwardFilterAlgorithmEx"));

   const auto tic = now();
   const auto res = orig_cudnnFindConvolutionBackwardFilterAlgorithmEx(handle, xDesc, x, dyDesc, y, convDesc, dwDesc, dw, requestedAlgoCount, returnedAlgoCount, perfResults, workSpace, workSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardFilterAlgorithm(
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardFilterAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardFilterAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionBackwardFilterAlgorithm(handle, xDesc, dyDesc, convDesc, dwDesc, preference, memoryLimitInBytes, algo);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardFilterAlgorithm_v7(
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardFilterAlgorithm_v7 = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardFilterAlgorithm_v7"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionBackwardFilterAlgorithm_v7(handle, srcDesc, diffDesc, convDesc, gradDesc, requestedAlgoCount, returnedAlgoCount, perfResults);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardFilterWorkspaceSize(
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardFilterWorkspaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardFilterWorkspaceSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionBackwardFilterWorkspaceSize(handle, xDesc, dyDesc, convDesc, gradDesc, algo, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnConvolutionBackwardFilter(
   ...
   static fun_t * orig_cudnnConvolutionBackwardFilter = (fun_t *) (dlsym(RTLD_NEXT,"cudnnConvolutionBackwardFilter"));

   const auto tic = now();
   const auto res = orig_cudnnConvolutionBackwardFilter(handle, alpha, xDesc, x, dyDesc, dy, convDesc, algo, workSpace, workSpaceSizeInBytes, beta, dwDesc, dw);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardDataAlgorithmMaxCount( cudnnHandle_t     handle,
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardDataAlgorithmMaxCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardDataAlgorithmMaxCount"));
...
cudnnStatus_t CUDNNWINAPI cudnnFindConvolutionBackwardDataAlgorithm(
   ...
   static fun_t * orig_cudnnFindConvolutionBackwardDataAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindConvolutionBackwardDataAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnFindConvolutionBackwardDataAlgorithm(handle, wDesc, dyDesc, convDesc, dxDesc, requestedAlgoCount, returnedAlgoCount, perfResults);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnFindConvolutionBackwardDataAlgorithmEx(
   ...
   static fun_t * orig_cudnnFindConvolutionBackwardDataAlgorithmEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindConvolutionBackwardDataAlgorithmEx"));

   const auto tic = now();
   const auto res = orig_cudnnFindConvolutionBackwardDataAlgorithmEx(handle, wDesc, w, dyDesc, dy, convDesc, dxDesc, dx, requestedAlgoCount, returnedAlgoCount, perfResults, workSpace, workSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardDataAlgorithm(
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardDataAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardDataAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionBackwardDataAlgorithm(handle, wDesc, dyDesc, convDesc, dxDesc, preference, memoryLimitInBytes, algo);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardDataAlgorithm_v7(
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardDataAlgorithm_v7 = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardDataAlgorithm_v7"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionBackwardDataAlgorithm_v7(handle, filterDesc, diffDesc, convDesc, gradDesc, requestedAlgoCount, returnedAlgoCount, perfResults);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetConvolutionBackwardDataWorkspaceSize(
   ...
   static fun_t * orig_cudnnGetConvolutionBackwardDataWorkspaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetConvolutionBackwardDataWorkspaceSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetConvolutionBackwardDataWorkspaceSize(handle, wDesc, dyDesc, convDesc, dxDesc, algo, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnConvolutionBackwardData(
   ...
   static fun_t * orig_cudnnConvolutionBackwardData = (fun_t *) (dlsym(RTLD_NEXT,"cudnnConvolutionBackwardData"));

   const auto tic = now();
   const auto res = orig_cudnnConvolutionBackwardData(handle, alpha, wDesc, w, dyDesc, dy, convDesc, algo, workSpace, workSpaceSizeInBytes, beta, dxDesc, dx);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSoftmaxForward(
   ...
   static fun_t * orig_cudnnSoftmaxForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSoftmaxForward"));

   const auto tic = now();
   const auto res = orig_cudnnSoftmaxForward(handle, algo, mode, alpha, xDesc, x, beta, yDesc, y);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSoftmaxBackward(
   ...
   static fun_t * orig_cudnnSoftmaxBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSoftmaxBackward"));

   const auto tic = now();
   const auto res = orig_cudnnSoftmaxBackward(handle, algo, mode, alpha, yDesc, y, dyDesc, dy, beta, dxDesc, dx);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreatePoolingDescriptor(
   ...
   static fun_t * orig_cudnnCreatePoolingDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreatePoolingDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetPooling2dDescriptor(
   ...
   static fun_t * orig_cudnnSetPooling2dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetPooling2dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetPooling2dDescriptor(poolingDesc, mode, maxpoolingNanOpt, windowHeight, windowWidth, verticalPadding, horizontalPadding, verticalStride, horizontalStride);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"arguments", {
       {"mode", convert_to_json(mode)},
              {"maxpoolingNanOpt", convert_to_json(maxpoolingNanOpt)},
              {"windowHeight", convert_to_json(windowHeight)},
              {"windowWidth", convert_to_json(windowWidth)},
              {"verticalPadding", convert_to_json(verticalPadding)},
              {"horizontalPadding", convert_to_json(horizontalPadding)},
              {"verticalStride", convert_to_json(verticalStride)},
              {"horizontalStride", convert_to_json(horizontalStride)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetPooling2dDescriptor(
   ...
   static fun_t * orig_cudnnGetPooling2dDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetPooling2dDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetPooling2dDescriptor(poolingDesc, mode, maxpoolingNanOpt, windowHeight, windowWidth, verticalPadding, horizontalPadding, verticalStride, horizontalStride);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetPoolingNdDescriptor(
   ...
   static fun_t * orig_cudnnSetPoolingNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetPoolingNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetPoolingNdDescriptor(poolingDesc, mode, maxpoolingNanOpt, nbDims, windowDimA, paddingA, strideA);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"mode", convert_to_json(mode)},
              {"maxpoolingNanOpt", convert_to_json(maxpoolingNanOpt)},
              {"nbDims", convert_to_json(nbDims)},
              {"windowDimA", convert_to_json(windowDimA)},
              {"paddingA", convert_to_json(paddingA)},
              {"strideA", convert_to_json(strideA)}
       }}
      };
      const auto str = js.dump();
...
cudnnStatus_t CUDNNWINAPI cudnnGetPoolingNdDescriptor(
   ...
   static fun_t * orig_cudnnGetPoolingNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetPoolingNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetPoolingNdDescriptor(poolingDesc, nbDimsRequested, mode, maxpoolingNanOpt, nbDims, windowDimA, paddingA, strideA);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetPoolingNdForwardOutputDim(
   ...
   static fun_t * orig_cudnnGetPoolingNdForwardOutputDim = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetPoolingNdForwardOutputDim"));

   const auto tic = now();
   const auto res = orig_cudnnGetPoolingNdForwardOutputDim(poolingDesc, inputTensorDesc, nbDims, outputTensorDimA);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"arguments", {
       {"nbDims", convert_to_json(nbDims)},
              {"outputTensorDimA", convert_to_json(outputTensorDimA)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetPooling2dForwardOutputDim(
   ...
   static fun_t * orig_cudnnGetPooling2dForwardOutputDim = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetPooling2dForwardOutputDim"));

   const auto tic = now();
   const auto res = orig_cudnnGetPooling2dForwardOutputDim(poolingDesc, inputTensorDesc, n, c, h, w);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyPoolingDescriptor(
   ...
   static fun_t * orig_cudnnDestroyPoolingDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyPoolingDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnPoolingForward(
   ...
   static fun_t * orig_cudnnPoolingForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnPoolingForward"));

   const auto tic = now();
   const auto res = orig_cudnnPoolingForward(handle, poolingDesc, alpha, xDesc, x, beta, yDesc, y);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnPoolingBackward(
   ...
   static fun_t * orig_cudnnPoolingBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnPoolingBackward"));

   const auto tic = now();
   const auto res = orig_cudnnPoolingBackward(handle, poolingDesc, alpha, yDesc, y, dyDesc, dy, xDesc, x, beta, dxDesc, dx);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateActivationDescriptor(
   ...
   static fun_t * orig_cudnnCreateActivationDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateActivationDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetActivationDescriptor(
   ...
   static fun_t * orig_cudnnSetActivationDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetActivationDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetActivationDescriptor(activationDesc, mode, reluNanOpt, coef);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"arguments", {
       {"mode", convert_to_json(mode)},
              {"reluNanOpt", convert_to_json(reluNanOpt)},
              {"coef", convert_to_json(coef)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetActivationDescriptor(
   ...
   static fun_t * orig_cudnnGetActivationDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetActivationDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetActivationDescriptor(activationDesc, mode, reluNanOpt, coef);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyActivationDescriptor(
   ...
   static fun_t * orig_cudnnDestroyActivationDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyActivationDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnActivationForward(
   ...
   static fun_t * orig_cudnnActivationForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnActivationForward"));

   const auto tic = now();
   const auto res = orig_cudnnActivationForward(handle, activationDesc, alpha, xDesc, x, beta, yDesc, y);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnActivationBackward(
   ...
   static fun_t * orig_cudnnActivationBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnActivationBackward"));

   const auto tic = now();
   const auto res = orig_cudnnActivationBackward(handle, activationDesc, alpha, yDesc, y, dyDesc, dy, xDesc, x, beta, dxDesc, dx);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateLRNDescriptor(
   ...
   static fun_t * orig_cudnnCreateLRNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateLRNDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetLRNDescriptor(
   ...
   static fun_t * orig_cudnnSetLRNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetLRNDescriptor"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"lrnN", convert_to_json(lrnN)},
              {"lrnAlpha", convert_to_json(lrnAlpha)},
              {"lrnBeta", convert_to_json(lrnBeta)},
              {"lrnK", convert_to_json(lrnK)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetLRNDescriptor(
   ...
   static fun_t * orig_cudnnGetLRNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetLRNDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyLRNDescriptor( cudnnLRNDescriptor_t lrnDesc ) {
   ...
   static fun_t * orig_cudnnDestroyLRNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyLRNDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnLRNCrossChannelForward(
   ...
   static fun_t * orig_cudnnLRNCrossChannelForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnLRNCrossChannelForward"));

   const auto tic = now();
   const auto res = orig_cudnnLRNCrossChannelForward(handle, normDesc, lrnMode, alpha, xDesc, x, beta, yDesc, y);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"lrnMode", convert_to_json(lrnMode)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnLRNCrossChannelBackward(
   ...
   static fun_t * orig_cudnnLRNCrossChannelBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnLRNCrossChannelBackward"));

   const auto tic = now();
   const auto res = orig_cudnnLRNCrossChannelBackward(handle, normDesc, lrnMode, alpha, yDesc, y, dyDesc, dy, xDesc, x, beta, dxDesc, dx);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDivisiveNormalizationForward(
   ...
   static fun_t * orig_cudnnDivisiveNormalizationForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDivisiveNormalizationForward"));

   const auto tic = now();
   const auto res = orig_cudnnDivisiveNormalizationForward(handle, normDesc, mode, alpha, xDesc, x, means, temp, temp2, beta, yDesc, y);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDivisiveNormalizationBackward(
   ...
   static fun_t * orig_cudnnDivisiveNormalizationBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDivisiveNormalizationBackward"));

   const auto tic = now();
   const auto res = orig_cudnnDivisiveNormalizationBackward(handle, normDesc, mode, alpha, xDesc, x, means, dy, temp, temp2, beta, dXdMeansDesc, dx, dMeans);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDeriveBNTensorDescriptor(
   ...
   static fun_t * orig_cudnnDeriveBNTensorDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDeriveBNTensorDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnBatchNormalizationForwardTraining(
   ...
   static fun_t * orig_cudnnBatchNormalizationForwardTraining = (fun_t *) (dlsym(RTLD_NEXT,"cudnnBatchNormalizationForwardTraining"));

   const auto tic = now();
   const auto res = orig_cudnnBatchNormalizationForwardTraining(handle, mode, alpha, beta, xDesc, x, yDesc, y, bnScaleBiasMeanVarDesc, bnScale, bnBias, exponentialAverageFactor, resultRunningMean, resultRunningVariance, epsilon, resultSaveMean, resultSaveInvVariance);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"arguments", {
       {"mode", convert_to_json(mode)},
              {"exponentialAverageFactor", convert_to_json(exponentialAverageFactor)},
              {"epsilon", convert_to_json(epsilon)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnBatchNormalizationForwardInference(
   ...
   static fun_t * orig_cudnnBatchNormalizationForwardInference = (fun_t *) (dlsym(RTLD_NEXT,"cudnnBatchNormalizationForwardInference"));

   const auto tic = now();
   const auto res = orig_cudnnBatchNormalizationForwardInference(handle, mode, alpha, beta, xDesc, x, yDesc, y, bnScaleBiasMeanVarDesc, bnScale, bnBias, estimatedMean, estimatedVariance, epsilon);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnBatchNormalizationBackward(
   ...
   static fun_t * orig_cudnnBatchNormalizationBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnBatchNormalizationBackward"));

   const auto tic = now();
   const auto res = orig_cudnnBatchNormalizationBackward(handle, mode, alphaDataDiff, betaDataDiff, alphaParamDiff, betaParamDiff, xDesc, x, dyDesc, dy, dxDesc, dx, dBnScaleBiasDesc, bnScale, dBnScaleResult, dBnBiasResult, epsilon, savedMean, savedInvVariance);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateSpatialTransformerDescriptor(
   ...
   static fun_t * orig_cudnnCreateSpatialTransformerDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateSpatialTransformerDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetSpatialTransformerNdDescriptor(
   ...
   static fun_t * orig_cudnnSetSpatialTransformerNdDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetSpatialTransformerNdDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetSpatialTransformerNdDescriptor(stDesc, samplerType, dataType, nbDims, dimA);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"samplerType", convert_to_json(samplerType)},
              {"dataType", convert_to_json(dataType)},
              {"nbDims", convert_to_json(nbDims)},
              {"dimA", convert_to_json(dimA)}
       }}
...
cudnnStatus_t CUDNNWINAPI cudnnDestroySpatialTransformerDescriptor(
   ...
   static fun_t * orig_cudnnDestroySpatialTransformerDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroySpatialTransformerDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSpatialTfGridGeneratorForward(
   ...
   static fun_t * orig_cudnnSpatialTfGridGeneratorForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSpatialTfGridGeneratorForward"));
...
cudnnStatus_t CUDNNWINAPI cudnnSpatialTfGridGeneratorBackward(
   ...
   static fun_t * orig_cudnnSpatialTfGridGeneratorBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSpatialTfGridGeneratorBackward"));

   const auto tic = now();
   const auto res = orig_cudnnSpatialTfGridGeneratorBackward(handle, stDesc, dgrid, dtheta);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSpatialTfSamplerForward(
   ...
   static fun_t * orig_cudnnSpatialTfSamplerForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSpatialTfSamplerForward"));

   const auto tic = now();
   const auto res = orig_cudnnSpatialTfSamplerForward(handle, stDesc, alpha, xDesc, x, grid, beta, yDesc, y);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSpatialTfSamplerBackward(
   ...
   static fun_t * orig_cudnnSpatialTfSamplerBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSpatialTfSamplerBackward"));

   const auto tic = now();
   const auto res = orig_cudnnSpatialTfSamplerBackward(handle, stDesc, alpha, xDesc, x, beta, dxDesc, dx, alphaDgrid, dyDesc, dy, grid, betaDgrid, dgrid);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateDropoutDescriptor(cudnnDropoutDescriptor_t * dropoutDesc) {
   ...
   static fun_t * orig_cudnnCreateDropoutDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateDropoutDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyDropoutDescriptor(cudnnDropoutDescriptor_t dropoutDesc) {
   ...
   static fun_t * orig_cudnnDestroyDropoutDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyDropoutDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnDropoutGetStatesSize(cudnnHandle_t handle, size_t * sizeInBytes) {
   ...
   static fun_t * orig_cudnnDropoutGetStatesSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDropoutGetStatesSize"));
...
cudnnStatus_t CUDNNWINAPI cudnnDropoutGetReserveSpaceSize(cudnnTensorDescriptor_t xdesc, size_t * sizeInBytes) {
   ...
   static fun_t * orig_cudnnDropoutGetReserveSpaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDropoutGetReserveSpaceSize"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetDropoutDescriptor(cudnnDropoutDescriptor_t dropoutDesc, 
   ...
   static fun_t * orig_cudnnSetDropoutDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetDropoutDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetDropoutDescriptor(dropoutDesc, handle, dropout, states, stateSizeInBytes, seed);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"stateSizeInBytes", convert_to_json(stateSizeInBytes)},
              {"seed", convert_to_json(seed)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnRestoreDropoutDescriptor(cudnnDropoutDescriptor_t dropoutDesc, 
   ...
   static fun_t * orig_cudnnRestoreDropoutDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnRestoreDropoutDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnRestoreDropoutDescriptor(dropoutDesc, handle, dropout, states, stateSizeInBytes, seed);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetDropoutDescriptor(cudnnDropoutDescriptor_t dropoutDesc, 
   ...
   static fun_t * orig_cudnnGetDropoutDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetDropoutDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetDropoutDescriptor(dropoutDesc, handle, dropout, states, seed);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDropoutForward(cudnnHandle_t                  handle, 
   ...
   static fun_t * orig_cudnnDropoutForward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDropoutForward"));

   const auto tic = now();
   const auto res = orig_cudnnDropoutForward(handle, dropoutDesc, xdesc, x, ydesc, y, reserveSpace, reserveSpaceSizeInBytes);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"reserveSpaceSizeInBytes", convert_to_json(reserveSpaceSizeInBytes)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnDropoutBackward(cudnnHandle_t                  handle, 
   ...
   static fun_t * orig_cudnnDropoutBackward = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDropoutBackward"));

   const auto tic = now();
   const auto res = orig_cudnnDropoutBackward(handle, dropoutDesc, dydesc, dy, dxdesc, dx, reserveSpace, reserveSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateRNNDescriptor(cudnnRNNDescriptor_t * rnnDesc) {
   ...
   static fun_t * orig_cudnnCreateRNNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateRNNDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyRNNDescriptor(cudnnRNNDescriptor_t rnnDesc) {
   ...
   static fun_t * orig_cudnnDestroyRNNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyRNNDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNForwardInferenceAlgorithmMaxCount(
   ...
   static fun_t * orig_cudnnGetRNNForwardInferenceAlgorithmMaxCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNForwardInferenceAlgorithmMaxCount"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNForwardInferenceAlgorithmMaxCount(handle, rnnDesc, count);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnFindRNNForwardInferenceAlgorithmEx( cudnnHandle_t handle,
   ...
   static fun_t * orig_cudnnFindRNNForwardInferenceAlgorithmEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindRNNForwardInferenceAlgorithmEx"));

   const auto tic = now();
   const auto res = orig_cudnnFindRNNForwardInferenceAlgorithmEx(handle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, cxDesc, cx, wDesc, w, yDesc, y, hyDesc, hy, cyDesc, cy, findIntensity, requestedAlgoCount, returnedAlgoCount, perfResults, workspace, workSpaceSizeInBytes);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"seqLength", convert_to_json(seqLength)},
              {"findIntensity", convert_to_json(findIntensity)},
              {"requestedAlgoCount", convert_to_json(requestedAlgoCount)},
              {"workSpaceSizeInBytes", convert_to_json(workSpaceSizeInBytes)}
       }}
      };
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNForwardTrainingAlgorithmMaxCount(
   ...
   static fun_t * orig_cudnnGetRNNForwardTrainingAlgorithmMaxCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNForwardTrainingAlgorithmMaxCount"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNForwardTrainingAlgorithmMaxCount(handle, rnnDesc, count);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnFindRNNForwardTrainingAlgorithmEx( cudnnHandle_t handle,
   ...
   static fun_t * orig_cudnnFindRNNForwardTrainingAlgorithmEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindRNNForwardTrainingAlgorithmEx"));

   const auto tic = now();
   const auto res = orig_cudnnFindRNNForwardTrainingAlgorithmEx(handle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, cxDesc, cx, wDesc, w, yDesc, y, hyDesc, hy, cyDesc, cy, findIntensity, requestedAlgoCount, returnedAlgoCount, perfResults, workspace, workSpaceSizeInBytes, reserveSpace, reserveSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNBackwardDataAlgorithmMaxCount(
   ...
   static fun_t * orig_cudnnGetRNNBackwardDataAlgorithmMaxCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNBackwardDataAlgorithmMaxCount"));
...
cudnnStatus_t CUDNNWINAPI cudnnFindRNNBackwardDataAlgorithmEx( cudnnHandle_t handle,
   ...
   static fun_t * orig_cudnnFindRNNBackwardDataAlgorithmEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindRNNBackwardDataAlgorithmEx"));
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNBackwardWeightsAlgorithmMaxCount(
   ...
   static fun_t * orig_cudnnGetRNNBackwardWeightsAlgorithmMaxCount = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNBackwardWeightsAlgorithmMaxCount"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNBackwardWeightsAlgorithmMaxCount(handle, rnnDesc, count);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnFindRNNBackwardWeightsAlgorithmEx( cudnnHandle_t handle,
   ...
   static fun_t * orig_cudnnFindRNNBackwardWeightsAlgorithmEx = (fun_t *) (dlsym(RTLD_NEXT,"cudnnFindRNNBackwardWeightsAlgorithmEx"));

   const auto tic = now();
   const auto res = orig_cudnnFindRNNBackwardWeightsAlgorithmEx(handle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, yDesc, y, findIntensity, requestedAlgoCount, returnedAlgoCount, perfResults, workspace, workSpaceSizeInBytes, dwDesc, dw, reserveSpace, reserveSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreatePersistentRNNPlan(cudnnRNNDescriptor_t       rnnDesc,
   ...
   static fun_t * orig_cudnnCreatePersistentRNNPlan = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreatePersistentRNNPlan"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"minibatch", convert_to_json(minibatch)},
              {"dataType", convert_to_json(dataType)}
       }}
      };
      const auto str = js.dump();
...
cudnnStatus_t CUDNNWINAPI cudnnSetPersistentRNNPlan(cudnnRNNDescriptor_t rnnDesc,
   ...
   static fun_t * orig_cudnnSetPersistentRNNPlan = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetPersistentRNNPlan"));
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyPersistentRNNPlan(cudnnPersistentRNNPlan_t plan) {
   ...
   static fun_t * orig_cudnnDestroyPersistentRNNPlan = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyPersistentRNNPlan"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetRNNDescriptor(cudnnHandle_t              handle,
   ...
   static fun_t * orig_cudnnSetRNNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetRNNDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnSetRNNDescriptor(handle, rnnDesc, hiddenSize, numLayers, dropoutDesc, inputMode, direction, mode, algo, dataType);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"hiddenSize", convert_to_json(hiddenSize)},
              {"numLayers", convert_to_json(numLayers)},
              {"inputMode", convert_to_json(inputMode)},
              {"direction", convert_to_json(direction)},
              {"mode", convert_to_json(mode)},
              {"algo", convert_to_json(algo)},
              {"dataType", convert_to_json(dataType)}
       }}
...
cudnnStatus_t CUDNNWINAPI cudnnSetRNNProjectionLayers(cudnnHandle_t        handle,
   ...
   static fun_t * orig_cudnnSetRNNProjectionLayers = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetRNNProjectionLayers"));

   const auto tic = now();
   const auto res = orig_cudnnSetRNNProjectionLayers(handle, rnnDesc, recProjSize, outProjSize);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"recProjSize", convert_to_json(recProjSize)},
              {"outProjSize", convert_to_json(outProjSize)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNProjectionLayers(cudnnHandle_t        handle,
   ...
   static fun_t * orig_cudnnGetRNNProjectionLayers = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNProjectionLayers"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNProjectionLayers(handle, rnnDesc, recProjSize, outProjSize);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetRNNAlgorithmDescriptor(cudnnHandle_t     handle,
   ...
   static fun_t * orig_cudnnSetRNNAlgorithmDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetRNNAlgorithmDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNDescriptor(cudnnHandle_t              handle,
   ...
   static fun_t * orig_cudnnGetRNNDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNDescriptor"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNDescriptor(handle, rnnDesc, hiddenSize, numLayers, dropoutDesc, inputMode, direction, mode, algo, dataType);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetRNNMatrixMathType(cudnnRNNDescriptor_t rnnDesc, cudnnMathType_t mType) {
   ...
   static fun_t * orig_cudnnSetRNNMatrixMathType = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetRNNMatrixMathType"));
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNMatrixMathType(cudnnRNNDescriptor_t rnnDesc, cudnnMathType_t* mType) {
   ...
   static fun_t * orig_cudnnGetRNNMatrixMathType = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNMatrixMathType"));
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNWorkspaceSize( cudnnHandle_t             handle,
   ...
   static fun_t * orig_cudnnGetRNNWorkspaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNWorkspaceSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNWorkspaceSize(handle, rnnDesc, seqLength, xDesc, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNTrainingReserveSize( cudnnHandle_t       handle,
   ...
   static fun_t * orig_cudnnGetRNNTrainingReserveSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNTrainingReserveSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNTrainingReserveSize(handle, rnnDesc, seqLength, xDesc, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNParamsSize(cudnnHandle_t                 handle,
   ...
   static fun_t * orig_cudnnGetRNNParamsSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNParamsSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNParamsSize(handle, rnnDesc, xDesc, sizeInBytes, dataType);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNLinLayerMatrixParams( cudnnHandle_t      handle,
   ...
   static fun_t * orig_cudnnGetRNNLinLayerMatrixParams = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNLinLayerMatrixParams"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNLinLayerMatrixParams(handle, rnnDesc, pseudoLayer, xDesc, wDesc, w, linLayerID, linLayerMatDesc, linLayerMat);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"pseudoLayer", convert_to_json(pseudoLayer)},
              {"linLayerID", convert_to_json(linLayerID)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetRNNLinLayerBiasParams( cudnnHandle_t        handle,
   ...
   static fun_t * orig_cudnnGetRNNLinLayerBiasParams = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetRNNLinLayerBiasParams"));

   const auto tic = now();
   const auto res = orig_cudnnGetRNNLinLayerBiasParams(handle, rnnDesc, pseudoLayer, xDesc, wDesc, w, linLayerID, linLayerBiasDesc, linLayerBias);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnRNNForwardInference( cudnnHandle_t             handle,
   ...
   static fun_t * orig_cudnnRNNForwardInference = (fun_t *) (dlsym(RTLD_NEXT,"cudnnRNNForwardInference"));

   const auto tic = now();
   const auto res = orig_cudnnRNNForwardInference(handle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, cxDesc, cx, wDesc, w, yDesc, y, hyDesc, hy, cyDesc, cy, workspace, workSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnRNNForwardTraining( cudnnHandle_t              handle,
   ...
   static fun_t * orig_cudnnRNNForwardTraining = (fun_t *) (dlsym(RTLD_NEXT,"cudnnRNNForwardTraining"));

   const auto tic = now();
   const auto res = orig_cudnnRNNForwardTraining(handle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, cxDesc, cx, wDesc, w, yDesc, y, hyDesc, hy, cyDesc, cy, workspace, workSpaceSizeInBytes, reserveSpace, reserveSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnRNNBackwardData( cudnnHandle_t                 handle,
   ...
   static fun_t * orig_cudnnRNNBackwardData = (fun_t *) (dlsym(RTLD_NEXT,"cudnnRNNBackwardData"));

   const auto tic = now();
   const auto res = orig_cudnnRNNBackwardData(handle, rnnDesc, seqLength, yDesc, y, dyDesc, dy, dhyDesc, dhy, dcyDesc, dcy, wDesc, w, hxDesc, hx, cxDesc, cx, dxDesc, dx, dhxDesc, dhx, dcxDesc, dcx, workspace, workSpaceSizeInBytes, reserveSpace, reserveSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnRNNBackwardWeights( cudnnHandle_t              handle,
   ...
   static fun_t * orig_cudnnRNNBackwardWeights = (fun_t *) (dlsym(RTLD_NEXT,"cudnnRNNBackwardWeights"));

   const auto tic = now();
   const auto res = orig_cudnnRNNBackwardWeights(handle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, yDesc, y, workspace, workSpaceSizeInBytes, dwDesc, dw, reserveSpace, reserveSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateCTCLossDescriptor( cudnnCTCLossDescriptor_t* ctcLossDesc ) {
   ...
   static fun_t * orig_cudnnCreateCTCLossDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateCTCLossDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetCTCLossDescriptor(
   ...
   static fun_t * orig_cudnnSetCTCLossDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetCTCLossDescriptor"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"compType", convert_to_json(compType)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetCTCLossDescriptor(
   ...
   static fun_t * orig_cudnnGetCTCLossDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetCTCLossDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyCTCLossDescriptor( cudnnCTCLossDescriptor_t ctcLossDesc ) {
   ...
   static fun_t * orig_cudnnDestroyCTCLossDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyCTCLossDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnCTCLoss( cudnnHandle_t handle, 
   ...
   const auto tic = now();
   const auto res = orig_cudnnCTCLoss(handle, probsDesc, probs, labels, labelLengths, inputLengths, costs, gradientsDesc, gradients, algo, ctcLossDesc, workspace, workSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnGetCTCLossWorkspaceSize(
   ...
   static fun_t * orig_cudnnGetCTCLossWorkspaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetCTCLossWorkspaceSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetCTCLossWorkspaceSize(handle, probsDesc, gradientsDesc, labels, labelLengths, inputLengths, algo, ctcLossDesc, sizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnCreateAlgorithmDescriptor(
   ...
   static fun_t * orig_cudnnCreateAlgorithmDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateAlgorithmDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnSetAlgorithmDescriptor(
   ...
   static fun_t * orig_cudnnSetAlgorithmDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetAlgorithmDescriptor"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"algorithm", convert_to_json(algorithm)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetAlgorithmDescriptor(
   ...
   static fun_t * orig_cudnnGetAlgorithmDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetAlgorithmDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnCopyAlgorithmDescriptor(
   ...
   static fun_t * orig_cudnnCopyAlgorithmDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCopyAlgorithmDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyAlgorithmDescriptor(
   ...
   static fun_t * orig_cudnnDestroyAlgorithmDescriptor = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyAlgorithmDescriptor"));
...
cudnnStatus_t CUDNNWINAPI cudnnCreateAlgorithmPerformance(
   ...
   static fun_t * orig_cudnnCreateAlgorithmPerformance = (fun_t *) (dlsym(RTLD_NEXT,"cudnnCreateAlgorithmPerformance"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"numberToCreate", convert_to_json(numberToCreate)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnSetAlgorithmPerformance(
   ...
   static fun_t * orig_cudnnSetAlgorithmPerformance = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetAlgorithmPerformance"));

   const auto tic = now();
   const auto res = orig_cudnnSetAlgorithmPerformance(algoPerf, algoDesc, status, time, memory);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"algoPerf", convert_to_json(algoPerf)},
              {"status", convert_to_json(status)},
              {"time", convert_to_json(time)},
              {"memory", convert_to_json(memory)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetAlgorithmPerformance(
   ...
   static fun_t * orig_cudnnGetAlgorithmPerformance = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetAlgorithmPerformance"));

   const auto tic = now();
   const auto res = orig_cudnnGetAlgorithmPerformance(algoPerf, algoDesc, status, time, memory);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnDestroyAlgorithmPerformance(
   ...
   static fun_t * orig_cudnnDestroyAlgorithmPerformance = (fun_t *) (dlsym(RTLD_NEXT,"cudnnDestroyAlgorithmPerformance"));
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"numberToDestroy", convert_to_json(numberToDestroy)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnGetAlgorithmSpaceSize(
   ...
   static fun_t * orig_cudnnGetAlgorithmSpaceSize = (fun_t *) (dlsym(RTLD_NEXT,"cudnnGetAlgorithmSpaceSize"));

   const auto tic = now();
   const auto res = orig_cudnnGetAlgorithmSpaceSize(handle, algoDesc, algoSpaceSizeInBytes);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSaveAlgorithm(
   ...
   static fun_t * orig_cudnnSaveAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSaveAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnSaveAlgorithm(handle, algoDesc, algoSpace, algoSpaceSizeInBytes);
   const auto toc = now();
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"algoSpaceSizeInBytes", convert_to_json(algoSpaceSizeInBytes)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnRestoreAlgorithm(
   ...
   static fun_t * orig_cudnnRestoreAlgorithm = (fun_t *) (dlsym(RTLD_NEXT,"cudnnRestoreAlgorithm"));

   const auto tic = now();
   const auto res = orig_cudnnRestoreAlgorithm(handle, algoSpace, algoSpaceSizeInBytes, algoDesc);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetCallback(
   ...
   if (is_record_time_enabled_q()) {
      const json js = {
       ...
       {"end", to_nanoseconds(toc)},
       {"arguments", {
       {"mask", convert_to_json(mask)},
              {"fptr", convert_to_json(fptr)}
       }}
      };
      const auto str = js.dump();
      record_cudnn_time(str.c_str());
...
cudnnStatus_t CUDNNWINAPI cudnnSetRNNDescriptor_v6(cudnnHandle_t         handle,
   ...
   static fun_t * orig_cudnnSetRNNDescriptor_v6 = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetRNNDescriptor_v6"));

   const auto tic = now();
   const auto res = orig_cudnnSetRNNDescriptor_v6(handle, rnnDesc, hiddenSize, numLayers, dropoutDesc, inputMode, direction, mode, algo, dataType);
   const auto toc = now();
...
cudnnStatus_t CUDNNWINAPI cudnnSetRNNDescriptor_v5(cudnnRNNDescriptor_t  rnnDesc,
   ...
   static fun_t * orig_cudnnSetRNNDescriptor_v5 = (fun_t *) (dlsym(RTLD_NEXT,"cudnnSetRNNDescriptor_v5"));

   const auto tic = now();
   const auto res = orig_cudnnSetRNNDescriptor_v5(rnnDesc, hiddenSize, numLayers, dropoutDesc, inputMode, direction, mode, dataType);
   const auto toc = now();
